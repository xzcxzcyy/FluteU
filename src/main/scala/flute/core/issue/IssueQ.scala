...
import flute.core.issue.AluDelayConfig.delayAmount
import flute.util.ValidBundle


object AluDelayConfig {
  val delayAmount = 1
}
...
class IQEntry extends Bundle {
  ...
  val selected   = Bool()
  val op1ShiftEn = Bool()
  val op1Shift   = SInt((delayAmount+1).W)  // op1Shift(0)即op1Ready
  val op2ShiftEn = Bool()
  val op2Shift   = SInt((delayAmount+1).W)  // op2Shift(0)即op2Ready
}
...
class IssueQ extends Module {
  ...
  val delayBus  = Wire(Vec(2, UInt((delayAmount+1).W)))  // delayBus也用dstBus的valid
