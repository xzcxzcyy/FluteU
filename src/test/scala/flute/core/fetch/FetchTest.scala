...
class FetchTest extends AnyFreeSpec with ChiselScalatestTester with Matchers {
  "Fetch instructions travelling test" in {
    test(new FetchTestTop) { c =>
      ...
      dp.insts(0).inst.expect(0x3801ffff.BM.U)
      dp.insts(1).inst.expect(0x380200ff.BM.U)
      ...
      c.clock.step()
      dp.instNum.expect(8.U)
      dp.insts(0).inst.expect(0xeeeeeeee.BM.U)
      dp.insts(1).inst.expect(0x00000001.U)
      dp.willProcess.poke(2.U)
      // another group in here...
      c.clock.step()
      dp.insts(0).inst.expect(0x2.U)
      dp.insts(1).inst.expect(0x3.U)
      dp.instNum.expect(6.U)
      dp.willProcess.poke(1.U)
      c.clock.step()
      dp.insts(0).inst.expect(0x3.U)
      dp.insts(1).inst.expect(0x4.U)
      dp.instNum.expect(8.U)
    }
  }
}
